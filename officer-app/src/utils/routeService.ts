...
function cleanupCache() {
  ...
  for (const [key, value] of ROUTE_CACHE.entries()) {
    if (now - value.timestamp > CACHE_TTL) ROUTE_CACHE.delete(key);
  }
...
function getCacheKey(lat1: number, lng1: number, lat2: number, lng2: number) {
  // Use 4 decimal places for cache key (approx 10m precision)
  return `${lat1.toFixed(4)},${lng1.toFixed(4)}->${lat2.toFixed(4)},${lng2.toFixed(4)}`;
}

/**
 * Parse an OSRM route response into our RouteData format.
 */
function parseRoute(route: any): RouteData {
  const coords: [number, number][] = route.geometry.coordinates.map(
    (c: [number, number]) => [c[1], c[0]] // OSRM returns [lng, lat], Leaflet needs [lat, lng]
  );
...
export function createRouteService() {
  ...
  async function fetchRoute(force: boolean = false): Promise<RouteData | null> {
    ...
    if (cached && (Date.now() - cached.timestamp < CACHE_TTL)) {
      console.log('📡 [ROUTE SERVICE] Using cached route');
    ...
    try {
      ...
      if (e.name !== 'AbortError') {
        console.warn('🟠 [ROUTE SERVICE] OSRM Fetch failed, retrying in 5s:', e.message);
        // On failure, keep showing old route but maybe notify or try fallback
      }
    }
  ...
  return {
    ...
    start(onRoute: (r: RouteData | null) => void, onLoading: (l: boolean) => void) {
      ...
      const handleSocketUpdate = (e: Event) => {
        ...
        if (detail && detail.coordinates) {
          const route = {
            ...
            nextStep: detail.nextStep || 'Proceed to target',
            raw: detail
          };
          state.cachedRoute = route;
          onRouteUpdate?.(route);
      ...
      window.addEventListener('navigation:update', handleSocketUpdate);
      (state as any)._socketHandler = handleSocketUpdate;
    ...
    stop() {
      if ((state as any)._socketHandler) {
        window.removeEventListener('navigation:update', (state as any)._socketHandler);
      }
      if (state.intervalId) {
